...
def _batch_extract(config, h5_file, batch_iterables):
    ...
    desc_factory = cv2.xfeatures2d.SIFT_create(config.nfeatures)
...
def _batch_extract_threaded(config, h5_file, sift_features_iter):
    ...
    with tqdm(total=sift_features_iter.count) as progress:
        ...
        parallel_for(sift_features_iter,
                     _batch_extract,
                     config,
                     h5_file,
...
def sift_features_load(config, sift_features_iter):
    """
    Utilitaire encapsulant extraction avec OpenCV ORB.

    config:
        Instance de SiftFeaturesConfig

    sift_features_iter:
        Instance de SiftFeaturesIter

    Retour:
        MetaObject encapsulant les features. Si le fichier demande existe,
        il est retourne sinon il est construit a partir de sift_features_iter.
    """
    def instantiate():
...
def sift_features_draw_key_points(sift_features, sift_features_iter):
    """
    ...
    sift_features:
        Object creee par sift_features_load()

    Retour:
        Image contenant les key points.
    """

    indices = sift_features.indices[...]

    for index, zip_info, image_reader in sift_features_iter:
        image = image_reader.get_image(zip_info)
        where = np.where(indices == index)
        key_points = sift_features.key_points[where][...]
